'use client'
import { getInfoMovie, viewAllSessions, getInfoSessions } from "@/app/plugins/communicationManager";
...
export default function Page() {
  ...
  useEffect(() => {
    (async () => {
      if (token) {
        const tickets = await verTicketsComprados();
        calcularPromedioVentas(tickets);
        const response = await allSesiones();
  ...
  function calcularPromedioVentas(tickets) {
    let total = 0;
    if (tickets.data) {
  ...
  function ticketsVenuts(sesion) {
    return sesion.seats.filter(seat => !seat.available);
  }

  function ticketsVips(sesion) {
    if (sesion.vip) {
      return sesion.seats.filter(seat => !seat.available && seat.row === 6);
    }
    return [];
  }

  function calculateTotal(sesion) {
    let total = 0;
    if (sesion.vip) {
      total = ticketsVips(sesion).length * 8 + ticketsVenuts(sesion).filter(seat => seat.row !== 6).length * 6;
    } else {
      total = ticketsVenuts(sesion).length * 6;
    }
    return total;
  }

  function ocupacio(sesion) {
    return (sesion.seats.filter(seat => !seat.available).length / 100 * sesion.seats.length).toFixed(1);
  }

  function ocupacioTotal() {
    let ocupacion = 0;
    sesions.map((sesion) => {
      ocupacion += parseFloat(ocupacio(sesion));
    });
    return (ocupacion / sesions.length).toFixed(1);
  }
  ...
  async function allSesiones() {
    try {
      const response = await viewAllSessions();
      response.data.sort((a, b) => {
        const timeA = new Date(`${a.date}T${a.time}:00`);
        const timeB = new Date(`${b.date}T${b.time}:00`);
        return timeA - timeB;
      });
      response.data.map((sesion) => {
        sesion.seats = JSON.parse(sesion.seats)
      });
        ...
        <div className="flex justify-center mt-5 mx-10">
          <table className="min-w-full">
            ...
            <tbody className="divide-y divide-gray-200">
              {!loading && sesions.map((sesion) => (
                <tr key={sesion.id} className="hover:bg-gray-200">
                  <td className="py-3 px-2">{sesion.title}</td>
                  <td className="py-3 px-2">10A</td>
                  <td className="py-3 px-2">{sesion.date}</td>
                  <td className="py-3 px-2">{sesion.time}</td>
                  <td className="py-3 px-2">{ticketsVenuts(sesion).length}</td>
                  <td className="py-3 px-2">{ocupacio(sesion)} %</td>
                  <td className="py-3 px-2">{calculateTotal(sesion)} €</td>
                  <td className="py-3 px-2">{estatActual(sesion)}</td>
                </tr>
              ))}
            </tbody>
          </table>
